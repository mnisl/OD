...
namespace OpenDental{
    ...
    "ersion only if you demand the latest features.  Be sure to update regularly.";
		...
		///<summary>Ryan is working on implementing this. It has been copied from SendAndReceiveXml() above.</summary>
		private static string RequestSNOMEDDownload() {
			...
			using(XmlWriter writer=XmlWriter.Create(strbuild,settings)) {
				...
				writer.WriteString(PrefC.GetString(PrefName.RegistrationKey));
				writer.WriteEndElement();
				writer.WriteStartElement("PracticeTitle");//now also includes address for requesting SNOMED
