...
namespace OpenDentBusiness.HL7 {
	...
	public class MessageParserMedLab {
		...
		private static string _msgArchiveFileName;
		...
		public static void ProcessZEF(HL7DefSegment segDef,List<SegmentHL7> listSegs) {
			...
			//Save to a temp pdf--------------------------------------------------------------------------
			string tempPathOD=CodeBase.ODFileUtils.CombinePaths(Path.GetTempPath(),"opendental");
			if(!Directory.Exists(tempPathOD)) {
				Directory.CreateDirectory(tempPathOD);
			}
			string tempPath=CodeBase.ODFileUtils.CombinePaths(tempPathOD,_patCur.PatNum.ToString()+".pdf");
			byte[] byteArray=Convert.FromBase64String(sb.ToString());
			File.WriteAllBytes(tempPath,byteArray);
			Document doc=new Document();
			doc.DocCategory=DefC.Short[(int)DefCat.ImageCats][0].DefNum;//put it in the first category.
			...
			try {
				doc=ImageStore.Import(tempPath,doc.DocCategory,_patCur);
			}
			catch(Exception ex) {
				EventLog.WriteEntry("MedLabHL7","Error saving pdf when processing a MedLab HL7 message.  "
					+"The pdf document may not be in the patient's image folder.\r\n"+ex.Message,EventLogEntryType.Information);
				return;
			}
			finally {
				//Clean up the temp file because there is no reason to leave files in the temp directory.
				try {
